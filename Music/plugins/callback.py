...
async def closesmex(_, CallbackQuery):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        await CallbackQuery.answer(
            "You're not allowed to close this Nigga", show_alert=True
        )
        return
...
async def resumevc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            await CallbackQuery.answer(
                "I dont think if something's paused on voice chat", show_alert=True
            )
            return
        else:
...
async def play_playlist(_, CallbackQuery):
    ...
    if str(smex) == "personal":
        ...
        if not _playlist:
            return await CallbackQuery.answer(
                f"You have no playlist on servers.", show_alert=True
            )
        else:
            await CallbackQuery.message.delete()
...
Personal Playlist Playing."""
    ...
    if str(smex) == "group":
        ...
        if not _playlist:
            return await CallbackQuery.answer(
                f"Your Group has no playlist on servers. Try adding musics in playlist.",
                show_alert=True,
            )
        else:
            await CallbackQuery.message.delete()
...
Group Playlist Playing."""
        ...
        if await isPreviewUp(preview):
            try:
                await CallbackQuery.message.reply_photo(
                    photo=preview,
                    caption=f"This is Queued Playlist of Your Group.\n\nIf you want to delete any music from playlist use : /delgroupplaylist",
                    quote=False,
                    reply_markup=key,
                )
                await m.delete()
...
async def group_playlist(_, CallbackQuery):
    ...
    if count == 30:
        return await CallbackQuery.message.reply_text(
            "Sorry! You can only have 30 music in group playlist."
        )
    try:
    ...
    if _check:
        return await CallbackQuery.message.reply_text(
            f"{Name}, Its already in the Playlist!"
        )
    assis = {
        "videoid": videoid,
        "title": title,
    ...
    Name = CallbackQuery.from_user.first_name
    return await CallbackQuery.message.reply_text(
        f"Added to Group's Playlist by {Name}"
    )
