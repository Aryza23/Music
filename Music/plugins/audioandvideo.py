...
async def getspy(_, CallbackQuery):
    ...
    try:
        if userLastDownloadTime > datetime.now():
            ...
            return await CallbackQuery.message.reply_text(
                f"Okay {Name}, Fast AF Boiii.. `Wait for {wait_time} min(s) before next download request"
            )
    except:
        pass
    ...
    try:
        ...
        now = datetime.now()
        user_time[userid] = now + timedelta(minutes=youtube_next_fetch)
    except Exception:
...
__Youtube Inline Download Powered By Aries Music__ """
    ...
    if med:
        loop.create_task(
            send_file(CallbackQuery, med, filename, videoid, user_id, link, channel)
        )
    else:
        print("med not found")
